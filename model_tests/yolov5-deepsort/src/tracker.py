...
#For some reason, the above section of code was not commented out, meaning that the tracker never read from the config file

class DeepSortTracker(): 
    def __new__(cls, config_path):
        return super(DeepSortTracker,cls).__new__(cls)
    ...
    def __init__(self, config_path):
        ...
        EMBEDDER_WTS = pkg_resources.resource_filename("deep_sort_realtime", "embedder/weights/mobilenetv2_bottleneck_wts.pt")           
        POLYGON = False              
        TODAY = None                 

        self.DISP_TRACKS = config['disp_tracks']
        self.DISP_OBJ_TRACK_BOX = config['disp_obj_track_box']
        self.OBJ_TRACK_COLOR = tuple(config['obj_tack_color'])
        self.OBJ_TRACK_BOX_COLOR = tuple(config['obj_track_box_color'])
        ...
        self.object_tracker = DeepSort(max_age=MAX_AGE,
                ...
                polygon=POLYGON,
                today=TODAY)
        print("TRACKER HAS BEEN INITIALISED\n\n\n\n\n\n")
    ...
    def display_track(self , track_history , tracks_current , img):
        ...
        for track in tracks_current:
            ...
            if prev_centers is not None and self.DISP_TRACKS == True:
